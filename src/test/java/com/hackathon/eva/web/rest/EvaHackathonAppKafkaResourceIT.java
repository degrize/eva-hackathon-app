...
import com.hackathon.eva.config.EmbeddedKafka;
import com.hackathon.eva.config.KafkaSseConsumer;
import com.hackathon.eva.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class EvaHackathonAppKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/eva-hackathon-app-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/eva-hackathon-app-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/eva-hackathon-app-kafka/unregister"));
                return;
            }
        }
